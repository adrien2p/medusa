...
const migrateRegions = async (mongodb, queryRunner) => {
  ...
  const regionRepository =
    queryRunner.manager.getCustomRepository(RegionRepository);
...
const createDiscount = async (mongodb, queryRunner, d) => {
  ...
  const discountRepo =
    queryRunner.manager.getCustomRepository(DiscountRepository);
...
const migrateOrders = async (mongodb, queryRunner) => {
  ...
  for (const o of orders) {
    ...
    const createLineItem = (li, custom = {}) => {
      let fulfilled_quantity = Math.min(
  ...
  await queryRunner.query(
    `ALTER SEQUENCE order_display_id_seq RESTART WITH ${
      parseInt(last.display_id) + 1
    }`
  );
};
...
const migrate = async () => {
  ...
  const { mongo_url, database_type, database_url, database_extra } =
